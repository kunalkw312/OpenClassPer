...
// ─── DOMContentLoaded: load initial data & attach listeners ───
document.addEventListener("DOMContentLoaded", () => {
    ...
    // URL param handling (legacy)
    const urlParams = new URLSearchParams(window.location.search);
    const path = urlParams.get('p');
    if (path) {
        // redirect or handle (optional)
        renderContent(path);
    }

    // date change listener (if elements exist)
    const startDateInput = document.getElementById("startDateInput");
    const planTypeSelect = document.getElementById("planType");
    if (startDateInput && planTypeSelect) {
...
// ──────────────────────────────────────────────
//  ADMIN PANEL FUNCTIONS (exposed globally)
// ──────────────────────────────────────────────
...
window.processPlanSave = async function () {
    ...
    alert("Plan Saved");
    loadInstitutePlans(); // refresh dropdown
...
// ── Legacy renderContent (placeholder) ──
function renderContent(path) {
    // This can be used for dynamic page loading if needed
    console.log("Legacy renderContent called with path:", path);
}
